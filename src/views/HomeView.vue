        ...
        <v-row>
          <v-col v-for="(obj, index) in objects" :key="index" cols="12" md="6" lg="4">
            <v-card>
              ...
              <v-card-title>
                {{ extractFileName(obj.filePath) }} <br />
                <small>Uploaded on {{ formatDate(extractTimestamp(obj.filePath)) }}</small>
              </v-card-title>
              <v-card-actions>
...
export default {
  ...
  methods: {
    ...
    extractFileName(filePath) {
      ...
      const filename = filenameWithTimestamp.substring(indexOfFirstHyphen + 1); // Everything after the first hyphen
      return filename;
    },
